...
in_memory_data = {
    "users": {},            # key = user_id_str, value = user data dict
    "stt_settings": {},     # key = user_id_str, value = {"language_code": ...}
    "pending_media": {},    # key = user_id_str, value = pending dict
}
...
# --------------------
# In-memory helpers (replaces DB usage)
# --------------------
def init_in_memory_data():
    # No DB to load from; leave empty or pre-populate if needed
    logging.info("Running with in-memory storage only (no MongoDB). Data will be lost on restart.")
...
# --------------------
# STT settings helpers (in-memory only)
# --------------------
...
def user_has_stt_setting(user_id: str) -> bool:
    setting = in_memory_data["stt_settings"].get(user_id)
    return bool(setting and setting.get("language_code"))
...
def save_pending_media(user_id: str, media_type: str, data: dict):
    """
    media_type: 'voice','audio','video','document','url'
    data: dict containing relevant keys. e.g. for file -> {'file_id': ..., 'chat_id':..., 'message_id':...}
          for url -> {'url': ..., 'chat_id':..., 'message_id':...}
    """
...
def start_select_lang_callback(call):
    ...
    if pending:
        ...
        if pending["media_type"] == "url":
            ...
            # file-based pending: media_type and file_id present
...
def admin_menu_callback(call):
    ...
    if call.data == "admin_total_users":
        ...
        bot.send_message(chat_id, f"📊 Total registered users (in-memory): *{total_registered}*", parse_mode="Markdown")
...
def broadcast_message(message):
    ...
    success = fail = 0
    # iterate over in-memory users keys
    for uid in list(in_memory_data["users"].keys()):
        if uid == str(ADMIN_ID):
            continue
        try:
...
def setup_bot():
    ...
    response += "Running in-memory only (no DB)."
    return response, 200
