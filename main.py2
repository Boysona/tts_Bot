...
def start_handler(message):
    ...
    markup = InlineKeyboardMarkup(row_width=2)
    # REMOVED "How it works" button as requested
    markup.add(
...
# NOTE: "How it works" quick-help callback removed entirely per request
...
def help_handler(message):
    ...
    if message.chat.type == 'private' and str(message.from_user.id) != str(ADMIN_ID) and not check_subscription(message.from_user.id):
        send_subscription_message(message.chat.id)
        return
    # Kept a minimal help message, no "How it works" detailed flow
    help_text = (
        "❓ *How to use*\n\n"
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if message.voice:
        ...
        if message.document.mime_type and (message.document.mime_type.startswith('audio/') or message.document.mime_type.startswith('video/')):
            ...
            # CLEARED: removed default warning message per user's request (no reply)
            return
    if not file_id:
        # CLEARED: removed default unsupported file reply per user's request (no reply)
        return
...
def handle_text_messages(message):
    ...
    # CLEARED: default reply removed per user's request (no unsolicited text reply)
    return

@bot.message_handler(func=lambda m: True, content_types=['sticker', 'photo'])
def handle_unsupported_media_types(message):
    update_user_activity_in_memory(message.from_user.id)
    # CLEARED: removed default unsupported media reply per user's request (no unsolicited reply)
    return
