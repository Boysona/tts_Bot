...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        if res['status'] == 'completed':
            ...
            # Friendly message for common transcoding/html errors (Drive public/private common case)
            if is_transcoding_like_error(error_msg):
                target_bot.send_message(chat_id, "❌ Transcription error: Fadlan hubi in link-ga uu yahay mid dadweyne (public) oo uu yahay file audio/video.", reply_to_message_id=original_message_id)
            else:
...
async def process_stt_media_url(chat_id: int, user_id_for_settings: str, url: str, target_bot: telebot.TeleBot, original_message_id: int):
    processing_msg = None
    try:
        # Only support Google Drive links
        if not is_google_drive_url(url):
            target_bot.send_message(chat_id, "⚠️ Waxaan kaliya ka jawaabaa Google Drive links. Fadlan soo dir link Google Drive ah (drive.google.com).", reply_to_message_id=original_message_id)
            return
        ...
        # If it's likely a Drive/publicity issue, give the shorter public-link hint
        err_str = str(e).lower()
...
def button_translate_handler(call):
    ...
    if uid not in user_transcriptions or message_id not in user_transcriptions[uid]:
        ...
        return
    # Build language selection keyboard and SEND it as a new message (don't edit original transcription)
    markup = InlineKeyboardMarkup(row_width=3)
    buttons = []
    for lang_name in STT_LANGUAGES.keys():
        # callback contains lang_name and original message id
...
def callback_translate_to(call):
    ...
    if uid not in user_transcriptions or message_id not in user_transcriptions[uid]:
        bot.answer_callback_query(call.id, "❌ Transcription expired")
        return
    # Remove the language-selection keyboard so it doesn't remain visible
    try:
        bot.edit_message_reply_markup(chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=None)
    except Exception as e:
        logging.info(f"Could not remove language selection keyboard: {e}")

    # Inform user via send_message (don't edit the original transcription)
    try:
...
def handle_text_messages(message):
    ...
    # If text contains a URL, assume user wants to transcribe that link (but only if Google Drive)
    url = extract_first_url(message.text or "")
    if url:
        uid = str(message.from_user.id)
        # Only respond to Google Drive links
        if not is_google_drive_url(url):
            bot.send_message(message.chat.id, "⚠️ Waxaan kaliya ka jawaabaa Google Drive links. Fadlan soo dir link Google Drive ah (drive.google.com).", reply_to_message_id=message.message_id)
            return
