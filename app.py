# bot.py
# Database removed — in-memory only version
import uuid
import logging
import requests
import telebot
...
# --------------------
in_memory_data = {
    # user_id_str -> { "_id": user_id_str, "first_seen": datetime, "last_active": datetime, "stt_conversion_count": int, ... }
    "users": {},
    # user_id_str -> { "language_code": "en" }
    "stt_settings": {},
}

user_transcriptions = {}  # user_id_str -> { message_id: transcription_text }
admin_state = {}
...
# --------------------
# In-memory helpers (replaced DB logic)
# --------------------
def init_in_memory_data():
    # Nothing to load — in-memory only.
    logging.info("Running in-memory only mode. No database initialized.")
...
# --------------------
# STT settings helpers (in-memory)
# --------------------
...
def set_stt_user_lang_in_memory(user_id: str, lang_code: str):
    in_memory_data["stt_settings"].setdefault(user_id, {})["language_code"] = lang_code
...
def broadcast_message(message):
    ...
    success = fail = 0
    # iterate over in-memory users
    for uid_str in list(in_memory_data["users"].keys()):
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    # Check in-memory for user setting. If not found, show language buttons (replying to the user's media message)
    if not user_has_stt_setting(user_id_for_settings):
        try:
            target_bot.send_message(
                message.chat.id,
                "first select Transcription language:",
