...
def get_tts_user_voice_in_memory(user_id: str) -> str:
    # Default voice is now Ava - English (United States) - Multilingual
    return in_memory_data["tts_settings"].get(user_id, {}).get("voice", "en-US-AvaMultilingualNeural")
...
def check_subscription(user_id: int) -> bool:
    # If REQUIRED_CHANNEL not set (empty), skip subscription enforcement.
...
def send_subscription_message(chat_id: int):
    # Only show join prompt if REQUIRED_CHANNEL was specified in the code (non-empty).
    if not REQUIRED_CHANNEL or not REQUIRED_CHANNEL.strip():
        return
    try:
...
def start_handler(message):
    ...
    welcome_message = (
        f"👋 Salaam {user_first_name}! I am your AI voice assistant that converts text to audio for free! 🔊✍️\n\n"
