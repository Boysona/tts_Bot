...
DB_PASSWORD = "<db_password>"  # ⚠️ Replace this with your actual database password
...
try:
    ...
    # Consider what to do if the database connection fails, e.g., exit or use in-memory fallback.
...
# No longer using in-memory data for persistence, but still need these for state management
...
def get_tts_user_settings(user_id: str) -> dict:
    return tts_settings_collection.find_one({"_id": user_id}) or {}

def set_tts_user_setting(user_id: str, key: str, value: any):
...
def get_tts_user_voice(user_id: str) -> str:
    settings = get_tts_user_settings(user_id)
    return settings.get("voice", "en-US-AvaMultilingualNeural")

def get_tts_user_pitch(user_id: str) -> int:
    settings = get_tts_user_settings(user_id)
    return settings.get("pitch", 0)

def get_tts_user_rate(user_id: str) -> int:
    settings = get_tts_user_settings(user_id)
    return settings.get("rate", 0)
...
def start_handler(message):
    ...
    bot.send_message(
        message.chat.id,
        "👋 Welcome! Choose the **language** of your voice. 👇",
...
def on_rate_set_callback(call):
    ...
    try:
        ...
        set_tts_user_setting(user_id, "rate", rate_value)
...
def on_pitch_set_callback(call):
    ...
    try:
        ...
        set_tts_user_setting(user_id, "pitch", pitch_value)
...
def on_tts_voice_change(call):
    ...
    set_tts_user_setting(user_id, "voice", voice)
...
def handle_text_for_tts_or_mode_input(message):
    ...
    if user_rate_input_mode.get(user_id) == "awaiting_rate_input":
        try:
            ...
            if -100 <= rate_val <= 100:
                set_tts_user_setting(user_id, "rate", rate_val)
    ...
    if user_pitch_input_mode.get(user_id) == "awaiting_pitch_input":
        try:
            ...
            if -100 <= pitch_val <= 100:
                set_tts_user_setting(user_id, "pitch", pitch_val)
