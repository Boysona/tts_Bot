...
DB_PASSWORD = os.getenv("DB_PASSWORD", "<db_password>") # Use environment variable
MONGO_URI = f"mongodb+srv://lakicalinuur:{DB_PASSWORD}@cluster0.n4hdlxk.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
DB_NAME = "SpeechBot"
...
TOKEN = os.getenv("TELEGRAM_TOKEN", "8281896922:AAEzRdF60SvTMyRba0ev4fzVvdY2ip-DStU")
ADMIN_ID = 6964068910
WEBHOOK_URL = os.getenv("WEBHOOK_URL", "https://tts-bot-lcn9.onrender.com")
REQUIRED_CHANNEL = "@guruubka_wasmada"

bot = telebot.TeleBot(TOKEN, threaded=True)
app = Flask(__name__)

# In-memory caches to reduce database reads
user_tts_mode = {}
...
def update_user_activity_in_db(user_id: int):
    ...
    users_collection.update_one(
        ...
        {"$set": {"last_active": now_iso},
         "$setOnInsert": {"tts_conversion_count": 0}},
        upsert=True
    )
    # in_memory_data["users"][user_id_str] = users_collection.find_one({"_id": user_id_str})
...
# --- Original functions, updated to use DB calls ---
...
def start_handler(message):
    ...
    update_user_activity_in_db(message.from_user.id) # Updated function call
    ...
    # Change the start message to directly show the voice selection
    bot.send_message(
        message.chat.id,
        "👋 Welcome! Let's get started. First, choose the *language* of your voice. 👇",
...
def admin_menu_callback(call):
    ...
    if call.data == "admin_total_users":
        total_registered = users_collection.count_documents({}) # Updated to count documents in the database
...
def broadcast_message(message):
    ...
    success = fail = 0
    # Iterate through users from the database
...
def on_rate_set_callback(call):
    ...
    update_user_activity_in_db(call.from_user.id) # Updated function call
    ...
    try:
        ...
        set_tts_user_rate_in_db(user_id, rate_value) # Updated function call
...
def on_pitch_set_callback(call):
    ...
    try:
        ...
        set_tts_user_pitch_in_db(user_id, pitch_value) # Updated function call
...
def on_tts_voice_change(call):
    ...
    set_tts_user_voice_in_db(user_id, voice) # Updated function call
    user_tts_mode[user_id] = voice
    current_pitch = get_tts_user_pitch_from_db(user_id) # Updated function call
    current_rate = get_tts_user_rate_from_db(user_id) # Updated function call
...
async def synth_and_send_tts(chat_id: int, user_id: str, text: str):
    voice = get_tts_user_voice_from_db(user_id) # Updated function call
    if voice.startswith("so-"):
        text = text.replace('.', ',')
    pitch_val = get_tts_user_pitch_from_db(user_id) # Updated function call
    rate_val = get_tts_user_rate_from_db(user_id) # Updated function call
    ...
    try:
        ...
        increment_processing_count_in_db(user_id, "tts") # Updated function call
    except Exception as e:
...
def handle_text_for_tts_or_mode_input(message):
    ...
    if user_rate_input_mode.get(user_id) == "awaiting_rate_input":
        try:
            ...
            if -100 <= rate_val <= 100:
                set_tts_user_rate_in_db(user_id, rate_val) # Updated function call
    ...
    if user_pitch_input_mode.get(user_id) == "awaiting_pitch_input":
        try:
            ...
            if -100 <= pitch_val <= 100:
                set_tts_user_pitch_in_db(user_id, pitch_val) # Updated function call
            ...
            bot.send_message(message.chat.id, "That is not a valid number. Send a number from -100 to +100 or 0. Try again:")
            return
    current_voice = get_tts_user_voice_from_db(user_id) # Updated function call
    if current_voice:
        threading.Thread(
