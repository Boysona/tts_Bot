...
# SQLite setup
# Use check_same_thread=False so connection can be used across threads,
# but protect access with db_lock for safety.
...
def ensure_tables():
    with db_lock:
        ...
        # users: id is string (telegram user id as text), times stored as ISO strings
        cur.execute("""
...
def init_in_memory_data():
    ...
    try:
        with db_lock:
            ...
            for row in users:
                uid = row["id"]
                # convert ISO strings back to datetime where possible
                first_seen = None
                last_active = None
                try:
                    if row["first_seen"]:
...
def update_user_activity_in_memory(user_id: int):
    ...
    with db_lock:
        cur = sqlite_conn.cursor()
        # Try insert; if exists, update last_active
        cur.execute("""
...
def increment_processing_count_in_memory(user_id: str, service_type: str):
    ...
    # Update SQLite - support only stt and tts for now
    if field_to_inc not in ("stt_conversion_count", "tts_conversion_count"):
        # fallback: ignore DB increment if unknown service_type
        with db_lock:
            cur = sqlite_conn.cursor()
            cur.execute("""
                INSERT OR IGNORE INTO users (id, first_seen, last_active, stt_conversion_count, tts_conversion_count)
...
# --------------------
# STT processing (unchanged - still uses AssemblyAI)
# --------------------
