...
def delete_transcription_later(user_id: str, message_id: int):
    time.sleep(3600)  # Changed to 1 hour
    with memory_lock:
...
def is_supported_url(url: str) -> bool:
    if not url:
        return False
    supported_domains = ["drive.google.com", "facebook.com", "fb.watch", "tiktok.com", "twitter.com", "x.com"]
...
def download_video_from_url(url: str) -> str | None:
    try:
        ...
        logging.error(f"Failed to download video from URL {url}: {e}")
        return None
...
def start_handler(message):
    ...
    bot.send_message(
        message.chat.id,
        "👋 Welcome! Send me your media file (audio, video, or voice message) or a supported link to get started!\n\n"
        "I can transcribe, translate, and summarize audio and video content up to 20MB. \n\n"
        "The current transcription language is **English 🇬🇧**. You can change it at any time using the /lang command. For best results, ensure the transcription language matches your audio file's language. 🌐",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("Change Language", callback_data="open_set_output_language"))
    )
...
def start_select_lang_callback(call):
    ...
    try:
        bot.edit_message_text(
            ...
            text=f"✅ Language set to **{lang_name}**. Now send your file or link.",
            parse_mode="Markdown"
        )
    except Exception as e:
        logging.info(f"Could not edit message after language selection: {e}")
        bot.send_message(
            call.message.chat.id,
            f"✅ Language set to **{lang_name}**. Now send your file or link.",
            parse_mode="Markdown"
        )
    ...
    if pending:
        ...
        if pending["media_type"] == "url":
            threading.Thread(target=process_stt_media_url, args=(pdata["chat_id"], uid, pdata["url"], bot, pdata["message_id"]), daemon=True).start()
        else:
            threading.Thread(target=process_stt_media, args=(pdata["chat_id"], uid, pending["media_type"], pdata["file_id"], bot, pdata["message_id"]), daemon=True).start()
...
def help_handler(message):
    ...
    help_text = (
        ...
        "• You can also send a link from **Facebook, TikTok, Twitter, or Google Drive**.\n"
        "• Use /lang or the inline button to choose the transcription language.\n"
...
def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int):
    processing_msg = None
    try:
        processing_msg = target_bot.send_message(chat_id, "Processing your file... Please wait a moment...", reply_to_message_id=original_message_id)
        ...
        try:
            target_bot.send_message(chat_id, "⚠️ The file is too large (max 20MB) or an error occurred. Please try a smaller file or upload to Google Drive and send the link.", reply_to_message_id=original_message_id)
        except Exception:
            pass
    finally:
        if processing_msg:
...
def process_stt_media_url(chat_id: int, user_id_for_settings: str, url: str, target_bot: telebot.TeleBot, original_message_id: int):
    processing_msg = None
    file_path = None
    try:
        processing_msg = target_bot.send_message(chat_id, "Processing your link...", reply_to_message_id=original_message_id)
        
        # Check if the URL is from a social media site
        if any(domain in url for domain in ["facebook.com", "fb.watch", "tiktok.com", "twitter.com", "x.com"]):
            target_bot.edit_message_text("Downloading video...", chat_id=chat_id, message_id=processing_msg.message_id)
            file_path = download_video_from_url(url)
            if not file_path:
                target_bot.edit_message_text("❌ Failed to download media from the provided link. Please check the URL and try again.", chat_id=chat_id, message_id=processing_msg.message_id)
                return
            
            # Upload the downloaded file to AssemblyAI
            target_bot.edit_message_text("Uploading and transcribing...", chat_id=chat_id, message_id=processing_msg.message_id)
            with open(file_path, "rb") as f:
                upload_res = requests.post("https://api.assemblyai.com/v2/upload", headers={"authorization": ASSEMBLYAI_API_KEY}, data=f)
                upload_res.raise_for_status()
                audio_url = upload_res.json()["upload_url"]
        else:
            # Assume it's a direct or Google Drive link
        ...
        try:
            target_bot.send_message(chat_id, "❌ An error occurred while processing your link. Ensure the link is a direct media link or a valid Google Drive/social media share link.", reply_to_message_id=original_message_id)
        except Exception:
            pass
    finally:
        if processing_msg:
...
def button_translate_handler(call):
    ...
    for i in range(0, len(buttons), 3):
        markup.add(*buttons[i:i+3])
    bot.answer_callback_query(call.id, "Please select the target language.")
...
def button_summarize_handler(call):
    ...
    for i in range(0, len(buttons), 3):
        markup.add(*buttons[i:i+3])
    bot.answer_callback_query(call.id, "Please select the summary language.")
...
def callback_translate_to(call):
    ...
    translating_msg = bot.send_message(call.message.chat.id, f"Translating to {lang_name}...", reply_to_message_id=message_id)
    ...
    def do_translate():
        ...
        if translated.startswith("Error:"):
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=translating_msg.message_id, text=f"❌ Translation error: {translated}")
            return
        if len(translated) > 4000:
            f = io.BytesIO(translated.encode("utf-8"))
            f.name = f"translation_{message_id}.txt"
            try:
                bot.send_document(call.message.chat.id, f, caption=f"Translation to {lang_name}:", reply_to_message_id=message_id)
            except Exception as e:
        ...
        try:
            bot.delete_message(translating_msg.chat.id, translating_msg.message_id)
        except Exception as e:
            logging.error(f"Could not delete translating message: {e}")
...
def callback_summarize_in(call):
    ...
    summarizing_msg = bot.send_message(call.message.chat.id, f"Summarizing in {lang_name}...", reply_to_message_id=message_id)

    def do_summarize():
        transcription = user_transcriptions[uid][message_id]
        summary = summarize_large_text_with_gemini(transcription, lang_name)
        if summary.startswith("Error:"):
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=summarizing_msg.message_id, text=f"❌ Summarization error: {summary}")
            return
        if len(summary) > 4000:
            f = io.BytesIO(summary.encode("utf-8"))
            f.name = f"summary_{message_id}.txt"
            try:
                bot.send_document(call.message.chat.id, f, caption=f"Summary in {lang_name}:", reply_to_message_id=message_id)
            except Exception as e:
        ...
        try:
            bot.delete_message(summarizing_msg.chat.id, summarizing_msg.message_id)
        except Exception as e:
            logging.error(f"Could not delete summarizing message: {e}")
...
def handle_text_messages(message):
    ...
    if url and is_supported_url(url):
        ...
        if not user_has_stt_setting(uid):
            ...
            bot.send_message(
                message.chat.id,
                "Please choose the transcription language first:",
                reply_markup=build_stt_language_keyboard(),
                reply_to_message_id=message.message_id
            )
            return
        threading.Thread(target=process_stt_media_url, args=(message.chat.id, uid, url, bot, message.message_id), daemon=True).start()
        return
    elif url and not is_supported_url(url):
        bot.send_message(message.chat.id, "❌ I can only transcribe links from Google Drive, Facebook, TikTok, and Twitter.", reply_to_message_id=message.message_id)
        return

    bot.send_message(message.chat.id, "I only transcribe audio/video files and supported links. Please send a file or a link.")
