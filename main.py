...
ASSEMBLYAI_API_KEY = "8615e9f175fc4a40b12cff5af9eca989"
...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int, file_size: int):
    processing_msg = None
    try:
        # Check file size before doing anything
        if file_size and file_size > 20 * 1024 * 1024:
            target_bot.send_message(chat_id, "⚠️ The file is too large. Please send one smaller than 20MB, or upload a link (Google Drive).", reply_to_message_id=original_message_id)
            return
        ...
        logging.info("Uploading stream to AssemblyAI...")
        
        # Use a streaming request to avoid loading the whole file into memory
        with requests.get(file_url, stream=True, timeout=30) as r:
            r.raise_for_status()  # Check for HTTP errors
            
            # AssemblyAI upload with streaming data
            upload_res = requests.post("https://api.assemblyai.com/v2/upload", headers=upload_headers, data=r.iter_content(chunk_size=524288), timeout=300)
            upload_res.raise_for_status()

        upload_json = upload_res.json()
        audio_url = upload_json.get('upload_url')
        if not audio_url:
            raise Exception(f"AssemblyAI upload failed: No upload_url received. Response: {upload_json}")

        lang_code = get_stt_user_lang_in_memory(user_id_for_settings)
        transcript_res = requests.post("https://api.assemblyai.com/v2/transcript", headers={"authorization": ASSEMBLYAI_API_KEY, "content-type": "application/json"}, json={"audio_url": audio_url, "language_code": lang_code, "speech_model": "best"}, timeout=60)
        ...
        target_bot.send_message(chat_id, "⚠️ An unexpected error occurred. Please try again or send a different file.", reply_to_message_id=original_message_id)
    finally:
        if processing_msg:
            try:
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if user_id_for_settings not in in_memory_data["stt_settings"]:
        ...
        return
    threading.Thread(target=lambda: asyncio.run(process_stt_media(message.chat.id, user_id_for_settings, message_type, file_id, target_bot, message.message_id, file_size))).start()
