...
def increment_processing_count_in_memory(user_id: str, service_type: str):
    """Fixed bug: ensure we use provided user_id parameter and convert to string"""
    user_id_str = str(user_id)
    now = datetime.now()
    user_data = in_memory_data["users"].get(user_id_str, {
        "_id": user_id_str,
...
# Improved welcome template with nicer UI (Markdown)
WELCOME_TEMPLATE = (
...
def start_handler(message):
    ...
    # Friendly welcome with language buttons
    ...
    # Add small language picker underneath
...
def help_quick_callback(call):
    help_text = (
        ...
        "If your file is larger than 20MB, upload it to Google Drive/Dropbox and send me the link.\n\n"
        "Need more help? /help"
    )
    try:
...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        target_bot.send_message(chat_id, "⚠️ Could not process the file. If it's large, please upload to Google Drive/Dropbox and send a link.", reply_to_message_id=original_message_id)
    finally:
        if processing_msg:
            try:
...
def download_youtube_audio(youtube_url: str):
    ...
    try:
        ...
        # yt-dlp command to extract best audio in mp3 format
        ydl_opts = {
...
def process_youtube_url(chat_id: int, user_id_for_settings: str, youtube_url: str, target_bot: telebot.TeleBot, original_message_id: int):
    processing_msg = None
    try:
        processing_msg = target_bot.send_message(chat_id, "🔄 Downloading YouTube audio...", reply_to_message_id=original_message_id)
        
        # Download audio from YouTube
        audio_file = download_youtube_audio(youtube_url)
        if not audio_file or not os.path.exists(audio_file):
            target_bot.send_message(chat_id, "❌ Failed to download audio from YouTube. Please check the URL and try again.", reply_to_message_id=original_message_id)
            return
            
        processing_msg = target_bot.edit_message_text("🔄 Processing YouTube audio...", chat_id, processing_msg.message_id)
        ...
        # Upload the downloaded audio file to AssemblyAI
        try:
            ...
            logging.exception(f"Unexpected error creating transcript from YouTube audio: {e}")
            target_bot.send_message(chat_id, "❌ Could not start transcription from the YouTube video.", reply_to_message_id=original_message_id)
            return
        finally:
        ...
        if res.get('status') == 'completed':
            text = res.get("text", "")
            if not text:
                target_bot.send_message(chat_id, "ℹ️ This video appears not to contain audible speech or it's unintelligible.", reply_to_message_id=original_message_id)
            elif len(text) <= 4000:
                user_transcriptions.setdefault(user_id_for_settings, {})[original_message_id] = text
                threading.Thread(target=delete_transcription_later, args=(user_id_for_settings, original_message_id), daemon=True).start()
                markup = InlineKeyboardMarkup()
                ...
                f.name = "transcript.txt"
                try:
                    target_bot.send_document(chat_id, f, caption="Here's your YouTube transcript", reply_to_message_id=original_message_id, reply_markup=markup)
                except Exception as e:
                    logging.error(f"Failed to send transcript file with buttons (YouTube): {e}")
                    try:
                        target_bot.send_document(chat_id, f, caption="YouTube transcription file:")
                    except Exception as ex:
                        logging.error(f"Failed fallback send (YouTube): {ex}")
            increment_processing_count_in_memory(user_id_for_settings, "stt")
        else:
            error_msg = res.get("error", "Unknown transcription error.")
            target_bot.send_message(chat_id, f"❌ Transcription error: {error_msg}", parse_mode="Markdown", reply_to_message_id=original_message_id)
            logging.error(f"AssemblyAI transcription failed (YouTube): {error_msg}")
    except requests.exceptions.RequestException as e:
        logging.error(f"Network or API error during YouTube processing: {e}")
        target_bot.send_message(chat_id, "❌ A network error occurred. Please try again.", reply_to_message_id=original_message_id)
    except Exception as e:
        logging.exception(f"Unhandled error during YouTube processing: {e}")
        target_bot.send_message(chat_id, "⚠️ Could not process the YouTube video.", reply_to_message_id=original_message_id)
    finally:
        if processing_msg:
            try:
                target_bot.delete_message(chat_id, processing_msg.message_id)
            except Exception as e:
                logging.error(f"Could not delete processing message (YouTube): {e}")
...
def handle_text_messages(message):
    ...
    if is_youtube_link(text):
        ...
        if uid not in in_memory_data["stt_settings"]:
            ...
            return
        threading.Thread(target=process_youtube_url, args=(message.chat.id, uid, text, bot, message.message_id), daemon=True).start()
        return
    
    # Detect common sharing links: Google Drive, Dropbox, direct media files
    ...
    # default reply for other texts (kept original behavior)
