...
in_memory_data = {
    "users": {},          # key = user_id_str -> dict with first_seen, last_active, stt_conversion_count, tts_conversion_count
...
def update_user_activity_in_memory(user_id: int):
    ...
    now = datetime.now()
    with memory_lock:
        user = in_memory_data["users"].get(user_id_str)
        if not user:
            user = {
                "_id": user_id_str,
                "first_seen": now,
        ...
        in_memory_data["users"][user_id_str] = user
...
def increment_processing_count_in_memory(user_id: str, service_type: str):
    ...
    with memory_lock:
        ...
        if service_type == "stt":
            user["stt_conversion_count"] = user.get("stt_conversion_count", 0) + 1
        elif service_type == "tts":
            user["tts_conversion_count"] = user.get("tts_conversion_count", 0) + 1
        user["last_active"] = now
        in_memory_data["users"][user_id_str] = user
...
def set_bot_info_and_startup():
    # No DB: initialize only webhook and commands
