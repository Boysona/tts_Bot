...
def is_supported_url(url: str) -> bool:
    return any(domain in url for domain in ['drive.google.com', 'facebook.com', 'tiktok.com', 'twitter.com', 'x.com'])
...
def get_direct_url_from_ytdlp(url: str):
    ydl_opts = {
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if 'entries' in info:
                # Handle playlists/multi-entry links
        ...
        logging.error(f"Error extracting URL with yt-dlp for {url}: {e}")
    return None
...
def help_handler(message):
    ...
    help_text = (
        ...
        "• You can also send a Google Drive, Facebook, TikTok, or Twitter link and I'll transcribe it.\n"
...
async def process_stt_media_url(chat_id: int, user_id_for_settings: str, url: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        if 'drive.google.com' in url:
            source_url = normalize_google_drive_url(url)
        elif any(domain in url for domain in ['facebook.com', 'tiktok.com', 'twitter.com', 'x.com']):
            source_url = get_direct_url_from_ytdlp(url)
        
        if not source_url:
            raise Exception("Could not extract a valid media URL from the provided link.")

        logging.info(f"Final media URL for AssemblyAI: {source_url}")
        ...
        try:
            target_bot.send_message(chat_id, "❌ An error occurred while processing your link. Ensure the link is from a supported source (Google Drive, Facebook, TikTok, Twitter) and is public.", reply_to_message_id=original_message_id)
        except Exception:
            pass
    finally:
        if processing_msg:
...
def button_translate_handler(call):
    ...
    bot.send_message(call.message.chat.id, " Select target language for translation:", reply_markup=markup, reply_to_message_id=call.message.message_id)
    bot.answer_callback_query(call.id)
...
def button_summarize_handler(call):
    ...
    bot.send_message(call.message.chat.id, "Select language for summary:", reply_markup=markup, reply_to_message_id=call.message.message_id)
    bot.answer_callback_query(call.id)
...
def callback_translate_to(call):
    ...
    bot.send_message(call.message.chat.id, f"Translating to {lang_name}...", reply_to_message_id=message_id)
    bot.answer_callback_query(call.id)
    ...
    def do_translate():
        ...
        if translated.startswith("Error:"):
            bot.send_message(call.message.chat.id, f"❌ Translation error: {translated}", reply_to_message_id=message_id)
            return
        if len(translated) > 4000:
            ...
            try:
                ...
                bot.send_message(call.message.chat.id, "✅ Translation complete (could not send file).", reply_to_message_id=message_id)
        else:
...
def callback_summarize_in(call):
    ...
    bot.send_message(call.message.chat.id, f"Summarizing in {lang_name}...", reply_to_message_id=message_id)
    bot.answer_callback_query(call.id)
    ...
    def do_summarize():
        ...
        if summary.startswith("Error:"):
            bot.send_message(call.message.chat.id, f"❌ Summarization error: {summary}", reply_to_message_id=message_id)
            return
        if len(summary) > 4000:
            ...
            try:
                ...
                bot.send_message(call.message.chat.id, "✅ Summary complete (could not send file).", reply_to_message_id=message_id)
        else:
...
def handle_text_messages(message):
    ...
    if url:
        ...
        if not is_supported_url(url):
            bot.send_message(message.chat.id, "⚠️ I only support links from Google Drive, Facebook, TikTok, and Twitter. Other links are not supported.", reply_to_message_id=message.message_id)
            return
