...
VOICE_MAPPING = {
    ...
    "mn-MN-YesuiNeural": "Yesui - Mongolian (Mongonia)",
    ...
    "en-US-BrianMultilingual2Neural": "Brian - English (United States) - Multilingual",
    ...
    "fr-FR-VivienneMultilingual2Neural": "Vivienne - French (France) - Multilingual",
    "fr-FR-RemyMultilingual2Neural": "Remy - French (France) - Multilingual",
    "fr-FR-LucienMultilingualNeural": "Lucien - French (France) - Multilingual",
    "de-DE-SeraphinaMultilingual2Neural": "Seraphina - German (Germany) - Multilingual",
    "de-DE-FlorianMultilingual2Neural": "Florian - German (Germany) - Multilingual",
    ...
    "it-IT-GiuseppeMultilingual2Neural": "Giuseppe - Italian (Italy) - Multilingual",
    ...
    "zh-CN-XiaoxiaoMultilingual2Neural": "Xiaoxiao - Chinese (Mandarin, Simplified) - Multilingual"
}
...
TTS_VOICES_BY_LANGUAGE = {
    ...
    "German": ["de-AT-IngridNeural", "de-AT-JonasNeural", "de-DE-AmalaNeural", "de-DE-ConradNeural", "de-DE-KatjaNeural", "de-DE-KillianNeural", "de-CH-JanNeural", "de-CH-LeniNeural"],
    "Greek": ["el-GR-AthinaNeural", "el-GR-NestorasNeural"],
    "English": [
        "en-AU-NatashaNeural", "en-CA-ClaraNeural", "en-CA-LiamNeural", "en-HK-YanNeural",
        "en-HK-SamNeural", "en-IN-NeerjaExpressiveNeural", "en-IN-NeerjaNeural",
        ...
        "en-GB-SoniaNeural", "en-GB-ThomasNeural", "en-US-AnaNeural", "en-US-AriaNeural",
        "en-US-ChristopherNeural", "en-US-EricNeural", "en-US-GuyNeural", "en-US-JennyNeural",
    ...
    "Spanish": [
        ...
        "es-ES-XimenaNeural", "es-CR-JuanNeural", "es-CR-MariaNeural", "es-CU-BelkysNeural",
        "es-CU-ManuelNeural", "es-DO-EmilioNeural", "es-DO-RamonaNeural", "es-EC-AndreaNeural",
        "es-EC-LuisNeural", "es-SV-LorenaNeural", "es-SV-RodrigoNeural", "es-GQ-JavierNeural",
        ...
        "es-NI-YolandaNeural", "es-PA-MargaritaNeural", "es-PA-RobertoNeural", "es-PY-MarioNeural",
        "es-PY-TaniaNeural", "es-PE-AlexNeural", "es-PE-CamilaNeural", "es-PR-KarinaNeural",
        "es-PR-VictorNeural", "es-ES-AlvaroNeural", "es-ES-ElviraNeural", "es-US-AlonsoNeural",
    ...
    "French": ["fr-BE-CharlineNeural", "fr-BE-GerardNeural", "fr-CA-ThierryNeural", "fr-CA-AntoineNeural", "fr-CA-JeanNeural", "fr-CA-SylvieNeural", "fr-FR-DeniseNeural", "fr-FR-EloiseNeural", "fr-FR-HenriNeural", "fr-CH-ArianeNeural", "fr-CH-FabriceNeural"],
    ...
    "Portuguese": ["pt-BR-AntonioNeural", "pt-BR-FranciscaNeural", "pt-PT-DuarteNeural", "pt-PT-RaquelNeural"],
    ...
    "Chinese": ["zh-HK-HiuGaaiNeural", "zh-HK-HiuMaanNeural", "zh-HK-WanLungNeural", "zh-CN-XiaoyiNeural", "zh-CN-YunjianNeural", "zh-CN-YunxiNeural", "zh-CN-YunxiaNeural", "zh-CN-YunyangNeural", "zh-CN-liaoning-XiaobeiNeural", "zh-TW-HsiaoChenNeural", "zh-TW-YunJheNeural", "zh-TW-HsiaoYuNeural", "zh-CN-shaanxi-XiaoniNeural"],
...
MULTILINGUAL_VOICES = [voice for voice, label in VOICE_MAPPING.items() if "Multilingual" in label]

def update_user_activity(user_id: int):
    user_id_str = str(user_id)
    now_iso = datetime.now().isoformat()
    users_collection.update_one({"_id": user_id_str}, {"$set": {"last_active": now_iso}, "$setOnInsert": {"_id": user_id_str, "tts_conversion_count": 0}}, upsert=True)

def increment_processing_count(user_id: str):
    user_id_str = str(user_id)
    users_collection.update_one({"_id": user_id_str}, {"$inc": {"tts_conversion_count": 1}, "$set": {"last_active": datetime.now().isoformat()}}, upsert=True)
...
def send_subscription_message(chat_id: int):
    ...
    try:
        ...
        bot.send_message(chat_id, "🔒 Access Locked You cannot use this bot until you join the Channel.", reply_markup=markup)
    except Exception as e:
...
def start_handler(message):
    ...
    bot.send_message(message.chat.id, "👋 Welcome! Choose how you want to select the voice. 👇", reply_markup=markup, parse_mode="Markdown")
...
def make_initial_choice_keyboard():
    ...
    markup.add(InlineKeyboardButton("🔥AI realistic any lang", callback_data="choice|auto"), InlineKeyboardButton("💗Natural single lang", callback_data="choice|by_language"))
    return markup

def _multilingual_label_for_button(voice_key: str) -> str:
    display = VOICE_MAPPING.get(voice_key, voice_key)
    base = display.split(" - ")[0].strip()
    if not base:
        base = voice_key.split("-")[-1].replace("MultilingualNeural", "").replace("Neural", "")
    return f"{base} speaks 90+ languages"

def make_multilingual_voices_keyboard():
    markup = InlineKeyboardMarkup(row_width=2)
    def sort_key(v):
        if v.startswith("en-US"):
            return ("0", v)
        if v.startswith("en-GB"):
            return ("1", v)
        return ("2", v)
    base_to_preferred = {}
    for voice in sorted(MULTILINGUAL_VOICES, key=sort_key):
        display = VOICE_MAPPING.get(voice, voice)
        base = display.split(" - ")[0].strip().lower()
        if base not in base_to_preferred:
            base_to_preferred[base] = voice
    for base, preferred_voice in base_to_preferred.items():
        label = _multilingual_label_for_button(preferred_voice)
        markup.add(InlineKeyboardButton(label, callback_data=f"tts_voice|{preferred_voice}"))
...
def handle_rate_command(message):
    ...
    bot.send_message(chat_id, "How fast or slow should I speak? Choose one or send a number from -100 (slow) to +100 (fast), 0 is normal:", reply_markup=make_rate_keyboard())
...
def on_rate_set_callback(call):
    ...
    try:
        ...
        bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text=f"🔊 Speech rate is *{rate_value}*.\n\nReady for text? Or use `/voice` to change the voice.", parse_mode="Markdown", reply_markup=None)
    except ValueError:
...
def handle_pitch_command(message):
    ...
    user_rate_input_mode[user_id] = None
    bot.send_message(chat_id, "Let's adjust the pitch! Choose one or send a number from -100 (low) to +100 (high), 0 is normal:", reply_markup=make_pitch_keyboard())
...
def on_pitch_set_callback(call):
    ...
    try:
        ...
        bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text=f"🔊 The pitch is *{pitch_value}*.\n\nReady for text? Or use `/voice` to change the voice.", parse_mode="Markdown", reply_markup=None)
    except ValueError:
...
def on_initial_choice(call):
    ...
    if choice == "auto":
        bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text="Choose a *multilingual* voice: 👇", reply_markup=make_multilingual_voices_keyboard(), parse_mode="Markdown")
    elif choice == "by_language":
        bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text="Choose the *language* of your voice. 👇", reply_markup=make_tts_language_keyboard(), parse_mode="Markdown")
    bot.answer_callback_query(call.id)
...
def on_back_to_initial_choice(call):
    ...
    user_pitch_input_mode[user_id] = None
    user_rate_input_mode[user_id] = None
    bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text="Choose how you want to select the voice. 👇", reply_markup=make_initial_choice_keyboard(), parse_mode="Markdown")
    bot.answer_callback_query(call.id)
...
def on_tts_language_select(call):
    ...
    bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text=f"Okay! Now choose a specific *voice* from {lang_name}. 👇", reply_markup=make_tts_voice_keyboard_for_language(lang_name), parse_mode="Markdown")
    bot.answer_callback_query(call.id)
...
def on_tts_voice_change(call):
    ...
    bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text=f"🔊 Great! You are using: *{voice_display_name}*.\n\nCurrent settings:\n• Pitch: *{current_pitch}*\n• Rate: *{current_rate}*\n\nReady to speak? Send me text!", parse_mode="Markdown", reply_markup=None)
...
async def synth_and_send_tts(chat_id: int, user_id: str, text: str):
    ...
    try:
        ...
        with open(filename, "rb") as f:
            voice_display_name = VOICE_MAPPING.get(voice, voice)
            caption_text = f"🎧 *Here is your voice!* \n\nVoice: **{voice_display_name}**\nPitch: *{pitch}*\nRate: *{rate}*\n\nEnjoy listening!✨ Follow me on TikTok: http://www.tiktok.com/@zack3d?lang=x-gent"
...
def handle_text_for_tts_or_mode_input(message):
    ...
    if current_voice:
        threading.Thread(target=lambda: asyncio.run(synth_and_send_tts(message.chat.id, user_id, message.text))).start()
    else:
...
def handle_unsupported_media_types(message):
    ...
    user_pitch_input_mode[user_id] = None
    user_rate_input_mode[user_id] = None
    bot.send_message(message.chat.id, "⚠️ I only convert *text* to audio. Send me text to convert to speech!")
