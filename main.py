...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        with requests.get(file_url, stream=True, timeout=30) as r:
            ...
            upload_res = requests.post("https://api.assemblyai.com/v2/upload", headers=upload_headers, data=chunk_generator(), timeout=300)
...
def handle_text_messages(message):
    ...
    if message.chat.type == 'private' and str(message.from_user.id) != str(ADMIN_ID) and not check_subscription(message.from_user.id):
        send_subscription_message(message.chat.id)
        return
    bot.send_message(message.chat.id, "I don't do text→audio. For converting text to speech use: https://t.me/Voice_maker_robot")
