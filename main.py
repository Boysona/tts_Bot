...
# ---------- REDUCED HD-ONLY VOICE LIST (as requested) ----------
# You asked to *only* add the HD voices (Dragon HD / HD Latest).
# These are the display names exactly as you provided. If edge_tts
# requires different short-names, we'll need to map them later.
VOICE_MAPPING = {
    # HD (Dragon HD Latest) - main set
    "Andrew Dragon HD Latest": "Andrew Dragon HD Latest",
    "Ava Dragon HD Latest": "Ava Dragon HD Latest",
    "Emma Dragon HD Latest": "Emma Dragon HD Latest",
    "Adam Dragon HD Latest": "Adam Dragon HD Latest",
    "Andrew2 Dragon HD Latest": "Andrew2 Dragon HD Latest",
    "Brian Dragon HD Latest": "Brian Dragon HD Latest",
    "Davis Dragon HD Latest": "Davis Dragon HD Latest",
    "Emma2 Dragon HD Latest": "Emma2 Dragon HD Latest",
    "Steffan Dragon HD Latest": "Steffan Dragon HD Latest",
    "Alloy Dragon HD Latest": "Alloy Dragon HD Latest",
    "Andrew3 Dragon HD Latest": "Andrew3 Dragon HD Latest",
    "Aria Dragon HD Latest": "Aria Dragon HD Latest",
    "Ava3 Dragon HD Latest": "Ava3 Dragon HD Latest",
    "Bree Dragon HD Latest": "Bree Dragon HD Latest",
    "Phoebe Dragon HD Latest": "Phoebe Dragon HD Latest",
    "Serena Dragon HD Latest": "Serena Dragon HD Latest",

    # Other languages - HD Latest (from your list)
    "Xiaochen Dragon HD Latest": "Xiaochen Dragon HD Latest",  # Chinese (Mandarin)
    "Remy Dragon HD Latest": "Remy Dragon HD Latest",          # French
    "Vivienne Dragon HD Latest": "Vivienne Dragon HD Latest",  # French
    "Florian Dragon HD Latest": "Florian Dragon HD Latest",    # German
    "Seraphina Dragon HD Latest": "Seraphina Dragon HD Latest",# German
    "Masaru Dragon HD Latest": "Masaru Dragon HD Latest",      # Japanese
    "Nanami Dragon HD Latest": "Nanami Dragon HD Latest",      # Japanese
}

# Grouping the HD voices under one visible "language" so the UI shows
# only the HD Latest voices as requested.
TTS_VOICES_BY_LANGUAGE = {
    "HD Latest": list(VOICE_MAPPING.keys())
}

# Only the HD Latest language in the ordering (so /voice shows HD only)
ORDERED_TTS_LANGUAGES = ["HD Latest"]

# Default voice (first HD voice) — used when no user setting exists.
DEFAULT_HD_VOICE = TTS_VOICES_BY_LANGUAGE["HD Latest"][0] if TTS_VOICES_BY_LANGUAGE["HD Latest"] else "Andrew Dragon HD Latest"
# ----------------------------------------------------------------

MOST_USED = ["HD Latest"]  # kept minimal
...
async def generate_and_send_voice_list(chat_id: int):
    try:
        # We'll still try to fetch available voices from edge_tts for info,
        # but the bot UI uses the reduced HD-only set above.
        voices_manager = await VoicesManager.create()
        voices = voices_manager.find()
        # Build a compact list that highlights the HD voices (if they exist in engine)
        output_lines = ["HD-ONLY voice list (bot UI) — display names below:\n"]
        output_lines.append("=" * 50 + "\n")
        for v in VOICE_MAPPING.keys():
            output_lines.append(f"{v}\n")
        output_lines.append("\n\n--- Full engine voices (sample, fetched) ---\n")
        # Add a few entries from engine if available (non-exhaustive)
        for voice in voices[:200]:
            output_lines.append(f"{voice.get('ShortName')} ({voice.get('Locale')})\n")
        ...
        with open(filename, 'rb') as f:
            bot.send_document(
                chat_id,
                f,
                caption="📋 HD-only voice list (plus sample engine voices)."
            )
        try:
            os.remove(filename)
...
async def synth_and_send_tts(chat_id: int, user_id: str, text: str):
    ...
    # Note: you provided display names like "Andrew Dragon HD Latest".
    # edge_tts may require an engine shortname (e.g., "en-US-AndrewNeural").
    # If synthesis fails due to voice name, we'll return a clear error message
    # so you can provide the correct shortname mapping.
    ...
    try:
        ...
        # Attempt to synthesize using the provided voice string.
        ...
        # Clear, helpful error for testing — likely indicates voice shortname mismatch.
        bot.send_message(chat_id, f"❌ Unexpected error during synthesis: {str(e)}\n\nPossible cause: the selected HD voice name may not match the engine's required shortname. If you get this, I can help map these display names to the correct edge_tts shortnames.")
    finally:
        stop_recording.set()
