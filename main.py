...
# New: pending media storage for when user hasn't selected language yet
pending_media = {}  # key: user_id_str -> dict with keys: type ('file'|'url'), chat_id, file_id/message_type/orig_message_id or url

# Corrected language flags and codes
STT_LANGUAGES = {
    "English 🇬🇧": "en",
    "العربية 🇸🇦": "ar",
    "Spanish 🇪🇸": "es",
...
def on_stt_language_select(call):
    ...
    # If there is pending media for this user, automatically start processing it
    pending = pending_media.pop(uid, None)
    try:
        # edit the keyboard message (if possible) to indicate selection
        try:
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f"✅ Selected language: *{lang_name}*\n\nProcessing your media...", parse_mode="Markdown", reply_markup=None)
        except Exception:
            bot.send_message(call.message.chat.id, f"✅ Selected language: *{lang_name}*\n\nProcessing your media...", parse_mode="Markdown")
    except Exception:
        pass

    if pending:
        # Start processing depending on type
        if pending.get("type") == "file":
            # Use stored file_id and message_type
            file_id = pending.get("file_id")
            message_type = pending.get("message_type")
            chat_id = pending.get("chat_id")
            orig_message_id = pending.get("orig_message_id")
            # Start processing in background
            threading.Thread(target=lambda: asyncio.run(process_stt_media(chat_id, uid, message_type, file_id, bot, orig_message_id)), daemon=True).start()
        elif pending.get("type") == "url":
            chat_id = pending.get("chat_id")
            url = pending.get("url")
            orig_message_id = pending.get("orig_message_id")
            threading.Thread(target=lambda: asyncio.run(process_stt_media_url(chat_id, uid, url, bot, orig_message_id)), daemon=True).start()
...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        # Build the Telegram file URL (this includes bot token so AssemblyAI can fetch it)
        file_url = f"https://api.telegram.org/file/bot{TOKEN}/{file_info.file_path}"
        logging.info(f"Using Telegram file URL directly (no proxy): {file_url}")

        # Use AssemblyAI transcript endpoint by passing audio_url directly (no upload)
...
def normalize_google_drive_url(url: str) -> str:
    ...
    """
    # patterns: https://drive.google.com/file/d/<id>/view?usp=sharing
    m = re.search(r'drive\.google\.com\/file\/d\/([a-zA-Z0-9_-]+)', url)
    if m:
        file_id = m.group(1)
        return f"https://drive.google.com/uc?export=download&id={file_id}"
    # shareable link pattern: https://drive.google.com/open?id=<id>
    m2 = re.search(r'drive\.google\.com\/open\?id=([a-zA-Z0-9_-]+)', url)
    if m2:
        file_id = m2.group(1)
        return f"https://drive.google.com/uc?export=download&id={file_id}"
    # already direct? return as-is
    return url
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if not file_id:
        ...
        return
    # If user hasn't set STT language, show keyboard and save pending media for auto-processing after selection
    if user_id_for_settings not in in_memory_data["stt_settings"]:
        try:
            sent = target_bot.send_message(message.chat.id, "Please choose transcription language first:", reply_to_message_id=message.message_id, reply_markup=build_stt_language_keyboard())
            # Save pending media
            pending_media[user_id_for_settings] = {
                "type": "file",
                "chat_id": message.chat.id,
                "file_id": file_id,
            ...
            # fallback: plain text prompt + keyboard
            try:
                ...
                pass
        return
    # Run processing in background thread (no proxy upload — we give AssemblyAI the Telegram file URL)
...
def callback_translate_to(call):
    ...
    # Send status message (will be deleted after translation completes)
    try:
        status_msg = bot.send_message(call.message.chat.id, f"Translating to {lang_name}...")
    except Exception:
        status_msg = None
    ...
    def do_translate():
        ...
        if status_msg:
            # attempt to delete the "Translating..." status once we have finished (or before sending results)
            try:
...
def callback_summarize_in(call):
    ...
    # Send status message (will be deleted after summarization completes)
    try:
        status_msg = bot.send_message(call.message.chat.id, f"Summarizing in {lang_name}...")
    except Exception:
        status_msg = None
...
def handle_text_messages(message):
    ...
    if url:
        ...
        if uid not in in_memory_data["stt_settings"]:
            # Send language keyboard and save pending url for processing after selection
            try:
                bot.send_message(message.chat.id, "Please choose transcription language first:", reply_to_message_id=message.message_id, reply_markup=build_stt_language_keyboard())
                pending_media[uid] = {
                    "type": "url",
                    "chat_id": message.chat.id,
                    "url": url,
