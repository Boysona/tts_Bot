...
# === CONFIG (keep your actual tokens/keys here) ===
...
# Max allowed by your bot logic (20MB)
...
WELCOME_TEMPLATE = (
    ...
    "• Tip: For files over 20MB or if your upload is restricted, upload to Google Drive/Dropbox and send the link.\n"
    "• I also accept direct links from *Facebook*, *TikTok*, and *X/Twitter* — I will try to extract a direct media URL.\n\n"
    "⚠️ Important: When sharing from Google Drive/Dropbox make sure the file/link sharing is set to: *Anyone with the link – Viewer*.\n\n"
    "🌐 Current language: *{lang_name}*\n"
    "🕒 Last update: 23/08/2025\n\n"
    "_Need help? Use /help or contact @kookabeela_"
)
...
def start_handler(message):
    ...
    markup.add(
        ...
        InlineKeyboardButton("How it works", callback_data="help_quick"),
    )
    markup.add(InlineKeyboardButton("Choose Input Language", callback_data="choose_input_lang"))
    try:
...
@bot.callback_query_handler(func=lambda c: c.data == "help_quick")
def help_quick_callback(call):
    help_text = (
        "❓ *Quick Help*\n\n"
        "• Send an audio message, audio file, or video (<= 20MB) and I'll transcribe it.\n"
        "• Use *Set Output Language* to choose which language the transcription should be in.\n"
        "• After transcription you'll have buttons to *Translate* or *Summarize*.\n\n"
        "Supported link sources: Google Drive, Dropbox, Facebook, TikTok, X/Twitter and direct media links (.mp3, .mp4, .m4a, .ogg, .webm).\n\n"
        "⚠️ For Google Drive/Dropbox: set file sharing to *Anyone with the link – Viewer*.\n"
        "If you provide a social link (Facebook/TikTok/X), I will try to extract a direct media file using yt-dlp. If extraction fails, please provide a direct downloadable link or upload the file directly.\n\n"
        "Need more help? /help"
    )
    try:
...
def help_handler(message):
    ...
    help_text = (
        ...
        "• If your file is >20MB, upload to Google Drive/Dropbox and set sharing to *Anyone with the link – Viewer* then send the link.\n"
        "• Facebook/TikTok/X links: I'll try to extract the direct media URL. Installing `yt-dlp` on the server improves success.\n\n"
...
def assemblyai_create_transcript_with_audio_url(audio_url: str, language_code: str):
    """Ask AssemblyAI to fetch the file directly from audio_url (preferred)."""
...
def assemblyai_upload_stream_and_create_transcript(stream_gen, language_code: str):
    """Fallback: stream upload to AssemblyAI (/upload), then create transcript from returned upload_url."""
...
# ---------- Social link helpers (yt-dlp) ----------
def is_social_link(text: str) -> bool:
    text = (text or "").lower()
    social_domains = ["facebook.com", "fb.watch", "tiktok.com", "vm.tiktok.com", "x.com", "twitter.com", "t.co", "instagram.com", "youtu.be", "youtube.com"]
    return any(domain in text for domain in social_domains)

def get_direct_media_via_ytdlp(url: str, timeout_seconds: int = 60) -> str:
    """
    Try to extract a direct media URL using:
      1) the yt-dlp binary (if installed)
      2) python yt_dlp module (if installed)
    Returns direct URL string or empty string on failure.
    """
    try:
        yt_dlp_bin = shutil.which("yt-dlp")
        if yt_dlp_bin:
            # -g prints direct URLs (may print video + audio) -> pick first
            cmd = [yt_dlp_bin, "-f", "bestaudio/best", "-g", url]
            logging.info(f"Running yt-dlp binary: {' '.join(cmd)}")
            out = subprocess.check_output(cmd, stderr=subprocess.STDOUT, timeout=timeout_seconds)
            txt = out.decode("utf-8").strip().splitlines()
            if txt:
                logging.info("yt-dlp returned direct URL(s).")
                return txt[0].strip()
    except subprocess.CalledProcessError as e:
        logging.warning(f"yt-dlp binary call failed: {e.output.decode('utf-8', errors='ignore') if e.output is not None else e}")
    except Exception as ex:
        logging.exception(f"Error running yt-dlp binary: {ex}")
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            logging.info("Using yt_dlp Python module to extract info.")
            info = ydl.extract_info(url, download=False)
            # If playlist/entries
            if isinstance(info, dict) and "url" in info and info.get("acodec", "") != "none":
                return info["url"]
            if isinstance(info, dict) and info.get("formats"):
                # Prefer audio or combined formats
                formats = info["formats"]
                # find best audio-like format url
                for fmt in reversed(formats):
                    # pick first format that has 'audio' or extension mp3/mp4 etc
                    if fmt.get("acodec") and fmt.get("url"):
                        return fmt["url"]
                # fallback to first format with url
                for fmt in formats:
                    if fmt.get("url"):
                        return fmt["url"]
            # sometimes 'entries' exist
        ...
        logging.exception(f"yt_dlp module extraction failed: {ex}")
...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        # If Telegram reports size and it's over our configured limit -> ask for link
        if getattr(file_info, "file_size", None) and file_info.file_size > MAX_FILE_BYTES:
            target_bot.send_message(
                chat_id,
                "⚠️ The file is too large (over 20MB). Please upload it to Google Drive or Dropbox and send a link.\n\n"
                "Suggestion: \"Upload a link (Google Drive/Dropbox).\"",
                reply_to_message_id=original_message_id
            )
            return

        # Construct a Telegram file URL (AssemblyAI can fetch it directly if accessible)
        ...
        # Preferred: let AssemblyAI fetch the Telegram file directly using audio_url (avoids proxying)
        try:
            transcript_resp = assemblyai_create_transcript_with_audio_url(file_url, lang_code)
        except requests.exceptions.RequestException as e:
            # If AssemblyAI can't fetch the URL (network/403), fallback to streaming upload
            logging.warning(f"AssemblyAI couldn't fetch file_url directly ({e}); falling back to streaming upload.")
            # Stream from Telegram and forward stream to AssemblyAI without fully loading to memory
        ...
        if res.get('status') == 'completed':
            ...
            if not text:
                ...
                # Long transcript -> send as a file but still include buttons
                user_transcriptions.setdefault(user_id_for_settings, {})[original_message_id] = text
                threading.Thread(target=delete_transcription_later, args=(user_id_for_settings, original_message_id), daemon=True).start()
                markup = InlineKeyboardMarkup()
                markup.add(
            ...
            # Error handling - detect common html/text errors and give friendly help
            ...
            if "text/html" in err_lower or "transcoding failed" in err_lower or "file does not appear to contain audio" in err_lower or "content-type" in err_lower:
                help_text = (
                    "❌ Could not fetch audio from that URL. Common fixes:\n\n"
                    "• If you sent a Google Drive or Dropbox link, set sharing to: *Anyone with the link – Viewer*.\n"
                    "• If you sent a Facebook/TikTok/X link, make sure the post is public or provide a direct/download link (the bot will try to extract it automatically if `yt-dlp` is available on the server).\n"
                    "• As a last resort, download the file yourself and upload it to the bot or a public direct link (ending with .mp3/.mp4/.m4a/etc.).\n\n"
                    "After fixing permissions, resend the link or file."
                )
                target_bot.send_message(chat_id, help_text, parse_mode="Markdown", reply_to_message_id=original_message_id)
            else:
        ...
        target_bot.send_message(chat_id, "⚠️ Could not process the file. If it's large or hosted, upload to Google Drive/Dropbox (Anyone with the link – Viewer) and send the link.", reply_to_message_id=original_message_id)
    finally:
        if processing_msg:
            try:
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if message.voice:
        ...
        if message.document.mime_type and (message.document.mime_type.startswith('audio/') or message.document.mime_type.startswith('video/')):
            ...
            target_bot.send_message(message.chat.id, "⚠️ I can only transcribe audio or video files. Send a valid file or a public link.")
            return
    if not file_id:
        target_bot.send_message(message.chat.id, "⚠️ Unsupported file type. Send audio, voice, or video.")
        return
    # If user hasn't set output language, prompt them
    ...
    # Start processing without loading entire file into memory
...
def extract_drive_direct_link(text: str) -> str:
    ...
    if m2:
        file_id = m2.group(1)
        return f"https://drive.google.com/uc?export=download&id={file_id}"
    # Some drive 'open?id=' or other forms - leave empty if not extractable
    return ""
...
def convert_dropbox_link(text: str) -> str:
    # Dropbox shared links often end with ?dl=0 — change to dl=1
    if "dl=0" in text:
        return text.replace("dl=0", "dl=1")
    if "dl=1" in text:
        return text
    # if no dl param, add raw=1
...
def process_external_audio_url(chat_id: int, user_id_for_settings: str, audio_url: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        # If user provided a social link (Facebook/TikTok/X) try to extract a direct URL using yt-dlp
        if is_social_link(audio_url) and not is_direct_media_url(audio_url):
            extracted = get_direct_media_via_ytdlp(audio_url)
            if extracted:
                logging.info(f"Extracted direct media URL via yt-dlp: {extracted}")
                audio_url = extracted
            else:
                # Give helpful instructions
                help_text = (
                    "⚠️ I couldn't automatically extract a direct media file from that social link.\n\n"
                    "Please try one of the following:\n"
                    "• Make the post public and resend the link (some social posts must be public).\n"
                    "• If possible, download the video/audio yourself and upload it to Google Drive/Dropbox and set sharing to *Anyone with the link – Viewer*, then send the link.\n"
                    "• Or upload the file directly to this chat (<= 20MB).\n\n"
                    "Tip: Installing `yt-dlp` on the bot server often allows automatic extraction of social links."
                )
                target_bot.send_message(chat_id, help_text, parse_mode="Markdown", reply_to_message_id=original_message_id)
                return

        # Attempt to create transcript with AssemblyAI
        try:
            ...
            # Try to proxy: GET the file and upload
        ...
        if res.get('status') == 'completed':
            ...
            if "text/html" in err_lower or "transcoding failed" in err_lower or "file does not appear to contain audio" in err_lower or "content-type" in err_lower:
                help_text = (
                    "❌ Could not fetch audio from that URL. Common fixes:\n\n"
                    "• If you sent a Google Drive or Dropbox link, set sharing to: *Anyone with the link – Viewer*.\n"
                    "• For Facebook/TikTok/X links: make sure the post is public or provide a direct/download link (yt-dlp may help extract it).\n"
                    "• Or download the file and upload it directly to the bot (<=20MB).\n\n"
                    "After fixing permissions, resend the link or file."
                )
                target_bot.send_message(chat_id, help_text, parse_mode="Markdown", reply_to_message_id=original_message_id)
            else:
...
def handle_text_messages(message):
    ...
    if is_google_drive_link(text):
        ...
        if not direct_link:
            bot.send_message(message.chat.id, "❌ Could not extract file ID from Google Drive link. Make sure it's a file-share link (format: /file/d/<id>/view or contains id=).")
            return
        uid = str(message.from_user.id)
    ...
    # Social links (Facebook/TikTok/X) - try to extract via yt-dlp
    if is_social_link(text):
        uid = str(message.from_user.id)
        if uid not in in_memory_data["stt_settings"]:
            bot.send_message(message.chat.id, "Please choose transcription language first using /lang or by selecting language after /start.", reply_markup=build_stt_language_keyboard())
            return
        # We'll attempt extraction inside process_external_audio_url, so call that
        threading.Thread(target=process_external_audio_url, args=(message.chat.id, uid, text, bot, message.message_id), daemon=True).start()
        return
