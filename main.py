...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        # Removed file size check (no 20MB enforcement)
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if not file_id:
        ...
        return
    # If user hasn't set language, show language keyboard automatically (attached to the incoming message)
    if user_id_for_settings not in in_memory_data["stt_settings"]:
        try:
            target_bot.send_message(message.chat.id, "🌍 Choose the transcription language:", reply_markup=build_stt_language_keyboard(), reply_to_message_id=message.message_id)
        except Exception:
            target_bot.send_message(message.chat.id, "🌍 Choose the transcription language:", reply_markup=build_stt_language_keyboard())
        return
...
def callback_translate_to(call):
    ...
    # Send a transient processing message and delete it after completion
    try:
        processing_msg = bot.send_message(chat_id=call.message.chat.id, text=f"Translating to {lang_name}...", reply_to_message_id=call.message.message_id)
    except Exception:
        processing_msg = None

    bot.answer_callback_query(call.id)

    def do_translate(processing_msg_obj):
        try:
            ...
            # delete the temporary processing message so it doesn't remain visible
            if processing_msg_obj:
                try:
                    bot.delete_message(call.message.chat.id, processing_msg_obj.message_id)
                except Exception:
                    pass

    threading.Thread(target=do_translate, args=(processing_msg,), daemon=True).start()
...
def callback_summarize_in(call):
    ...
    # Send a transient processing message and delete it after completion
    try:
        processing_msg = bot.send_message(chat_id=call.message.chat.id, text=f"Summarizing in {lang_name}...", reply_to_message_id=call.message.message_id)
    except Exception:
        processing_msg = None

    bot.answer_callback_query(call.id)

    def do_summarize(processing_msg_obj):
        try:
            transcription = user_transcriptions[uid][message_id]
    ...
    threading.Thread(target=do_summarize, args=(processing_msg,), daemon=True).start()
...
def handle_text_messages(message):
    ...
    if url:
        ...
        if uid not in in_memory_data["stt_settings"]:
            # Show the language keyboard automatically (attached to the message) instead of the plain instructive text.
            try:
                bot.send_message(message.chat.id, "🌍 Choose the transcription language:", reply_markup=build_stt_language_keyboard(), reply_to_message_id=message.message_id)
            except Exception:
                bot.send_message(message.chat.id, "🌍 Choose the transcription language:", reply_markup=build_stt_language_keyboard())
            return
        # Removed the "🔗 Link detected..." message; process_stt_media_url will send the processing notice.
