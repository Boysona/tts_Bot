...
def start_handler(message):
    ...
    # Replaced the broken "Choose Input Language" callback with an active "Add me to your Group" URL button
...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        # Quick HEAD-check to ensure Telegram file_url returns audio content (avoid HTML proxies)
        try:
            head = requests.head(file_url, allow_redirects=True, timeout=10)
            ct = (head.headers.get("content-type") or "").lower()
            if "text/html" in ct:
                target_bot.send_message(chat_id, "❌ The Telegram file link appears to return HTML instead of audio. Try resending the file or upload it to Google Drive/Dropbox and share a direct link (set sharing to *Anyone with the link*).", parse_mode="Markdown", reply_to_message_id=original_message_id)
                return
        except Exception:
            # if head fails, proceed and let AssemblyAI attempt to fetch / we'll catch errors later
            pass
        ...
        target_bot.send_message(chat_id, "⚠️ The file is too large (over 20MB). Please sand me smaller 20MB file or upload it to Google Drive or Dropbox and send a link.", reply_to_message_id=original_message_id)
    finally:
        if processing_msg:
            try:
...
def process_external_audio_url(chat_id: int, user_id_for_settings: str, audio_url: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        # --- NEW: HEAD-check the URL to ensure it returns audio content, not an HTML preview page ---
        try:
            head = requests.head(audio_url, allow_redirects=True, timeout=10)
            ct = (head.headers.get("content-type") or "").lower()
            if "text/html" in ct:
                # Helpful message instead of raw AssemblyAI error
                target_bot.send_message(
                    chat_id,
                    "❌ The link you sent appears to return an HTML page (a preview), not an audio file.\n\n"
                    "Please make the file public and set sharing to *\"Anyone with the link\"* and then send one of these:\n"
                    "• A Google Drive link in the format: `/file/d/<FILE_ID>/view` (or include `id=<FILE_ID>`), *and* share the file as Anyone with the link (Viewer).\n"
                    "• Or use a direct download link (Drive `uc?export=download&id=...`, Dropbox with `dl=1` or `raw=1`).\n\n"
                    "How to: In Google Drive → Right click file → Share → Change to *Anyone with the link* → Copy link.",
                    parse_mode="Markdown",
                    reply_to_message_id=original_message_id
                )
                return
        except Exception:
            # if HEAD fails, we'll still try to proceed - AssemblyAI or stream upload will catch issues
            pass
        ...
        try:
            ...
            with requests.get(audio_url, stream=True, timeout=60) as r:
                r.raise_for_status()
                # Another check: if the GET returns HTML page content-type, inform user
                ct_get = (r.headers.get("content-type") or "").lower()
                if "text/html" in ct_get:
                    target_bot.send_message(
                        chat_id,
                        "❌ The link returned an HTML page when fetched. Please set sharing to *Anyone with the link* or provide a direct file link (Drive/Dropbox direct download).",
                        parse_mode="Markdown",
                        reply_to_message_id=original_message_id
                    )
                    return
...
def handle_unsupported_media_types(message):
    update_user_activity_in_memory(message.from_user.id)
    bot.send_message(message.chat.id, "⚠️ I only transcribe audio or video. Send an audio message, audio file, or a video (up 20MB).")
