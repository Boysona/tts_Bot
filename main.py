...
# NOTE: /help and /privacy handlers removed per user request (entirely deleted).
...
def make_tts_language_keyboard():
    """
    Language selection keyboard used in /start and /voice.
    Includes the two bottom buttons: Auto and By Language.
    """
    ...
    # Add the requested two bottom buttons
    bottom_row = [
        InlineKeyboardButton("Auto", callback_data="tts_mode|auto"),
        InlineKeyboardButton("By Language", callback_data="tts_mode|bylang")
    ]
    markup.add(*bottom_row)
    return markup

def make_tts_language_only_keyboard():
    """
    Language-only keyboard (used after pressing By Language) - no Auto/By Language buttons.
    """
    markup = InlineKeyboardMarkup(row_width=3)
    buttons = []
    for lang_name in ORDERED_TTS_LANGUAGES:
    ...
    for i in range(0, len(buttons), 3):
        markup.add(*buttons[i:i+3])
    markup.add(InlineKeyboardButton("⬅️ Back", callback_data="tts_back_to_main"))
    return markup

def make_multilingual_keyboard():
    """
    Create keyboard that shows all voices that are Multilingual (Auto mode).
    """
    markup = InlineKeyboardMarkup(row_width=2)
    multilingual_voices = [v for v in VOICE_MAPPING.keys() if "Multilingual" in v or "Multilingual" in VOICE_MAPPING.get(v, "")]
    multilingual_voices_sorted = sorted(multilingual_voices)
    for voice in multilingual_voices_sorted:
...
def make_tts_voice_keyboard_for_language(lang_name: str):
    """
    Show voices for a given language; exclude Multilingual voices here as requested.
    """
    markup = InlineKeyboardMarkup(row_width=2)
    voices = TTS_VOICES_BY_LANGUAGE.get(lang_name, [])
    for voice in voices:
        # Exclude voices that are Multilingual (either by id or mapping label)
        if "Multilingual" in voice or "Multilingual" in VOICE_MAPPING.get(voice, ""):
            continue
        display_name = VOICE_MAPPING.get(voice, voice)
        markup.add(InlineKeyboardButton(display_name, callback_data=f"tts_voice|{voice}"))
...
@bot.callback_query_handler(lambda c: c.data.startswith("tts_mode|"))
def on_tts_mode_select(call):
    """
    Handle the bottom two buttons:
    - tts_mode|auto -> show multilingual voices
    - tts_mode|bylang -> show languages-only keyboard
    """
    user_id = str(call.from_user.id)
    update_user_activity(call.from_user.id)
    if call.message.chat.type == 'private' and str(call.from_user.id) != str(ADMIN_ID) and not check_subscription(call.message.chat.id):
    ...
    if mode == "auto":
        bot.edit_message_text(
            chat_id=chat_id,
            message_id=call.message.message_id,
            text="Auto mode — choose any *Multilingual* voice. 👇",
            reply_markup=make_multilingual_keyboard(),
            parse_mode="Markdown"
        )
    elif mode == "bylang":
        bot.edit_message_text(
            chat_id=chat_id,
            message_id=call.message.message_id,
            text="Choose the *language* you want (By Language). 👇",
            reply_markup=make_tts_language_only_keyboard(),
            parse_mode="Markdown"
        )
    bot.answer_callback_query(call.id)
...
def on_tts_language_select(call):
    ...
    bot.edit_message_text(
        chat_id=chat_id,
        message_id=call.message.message_id,
        text=f"Okay! Now choose a specific *voice* from {lang_name}. 👇 (Multilingual voices are excluded here.)",
...
@bot.callback_query_handler(lambda c: c.data == "tts_back_to_main")
def on_tts_back_to_main(call):
    """
    Back to the main keyboard that contains Auto / By Language as bottom buttons.
    """
    user_id = str(call.from_user.id)
    update_user_activity(call.from_user.id)
    if call.message.chat.type == 'private' and str(call.from_user.id) != str(ADMIN_ID) and not check_subscription(call.message.chat.id):
    ...
    bot.edit_message_text(
        chat_id=chat_id,
        message_id=call.message.message_id,
        text="Choose the voice actor from the options below. 👇",
        reply_markup=make_tts_language_keyboard(),
        parse_mode="Markdown"
    )
    bot.answer_callback_query(call.id)
