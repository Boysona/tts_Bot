...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        # If Telegram reports size > 20MB, inform user (only once)
        ...
        try:
            target_bot.send_message(chat_id, "❌ An error occurred while processing your file. Please try again or send a smaller/clearer file.", reply_to_message_id=original_message_id)
        except Exception:
            pass
    finally:
        if processing_msg:
...
def handle_text_messages(message):
    ...
    if url:
        ...
        if uid not in in_memory_data["stt_settings"]:
            bot.send_message(message.chat.id, "Please choose transcription language first using /lang or by selecting language after /start.")
            return
        bot.send_message(message.chat.id, "🔗 Link detected. Attempting to transcribe the provided link...")
