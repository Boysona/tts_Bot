...
def extract_google_drive_url(text: str) -> str:
    # This regex is specifically for Google Drive public file URLs
    m = re.search(r'(https?://(?:www\.)?drive\.google\.com/(?:file/d/|open\?id=)[a-zA-Z0-9_-]+)', text)
    if m:
        return m.group(1)
    # Check for direct file download links which may be used
    m2 = re.search(r'(https?://(?:www\.)?drive\.google\.com/uc\?export=download&id=[a-zA-Z0-9_-]+)', text)
    if m2:
        return m2.group(1)
    return None
...
def start_handler(message):
    ...
    bot.send_message(
        message.chat.id,
        "👋 Welcome! I'm your transcription bot. I can transcribe your audio and video files. Use the buttons below to select your transcription language:",
...
def start_select_lang_callback(call):
    ...
    try:
        bot.edit_message_text(
            ...
            text=f"✅ Language set to **{lang_name}**. Now send me your audio or video file, or a Google Drive link to a media file.",
            parse_mode="Markdown"
        )
    except Exception:
        bot.send_message(
            call.message.chat.id,
            f"✅ Language set to **{lang_name}**. Now send me your audio or video file, or a Google Drive link to a media file.",
            parse_mode="Markdown"
        )
...
def help_handler(message):
    ...
    help_text = (
        ...
        "• You can also send a Google Drive link and I'll transcribe it.\n"
        "• Use /lang to choose the transcription language.\n"
...
def on_stt_language_select(call):
    ...
    try:
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text=f"✅ Language set to **{lang_name}**. Now send me your file or a Google Drive link.",
            parse_mode="Markdown"
        )
    except Exception:
        bot.send_message(
            call.message.chat.id,
            f"✅ Language set to **{lang_name}**. Now send me your file or a Google Drive link.",
            parse_mode="Markdown"
        )
...
async def process_stt_media_url(chat_id: int, user_id_for_settings: str, url: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        if not normalized:
            target_bot.send_message(chat_id, "❌ I only support Google Drive links for now. Please send a valid Google Drive link.", reply_to_message_id=original_message_id)
            return
        ...
        try:
            target_bot.send_message(chat_id, "❌ An error occurred while processing your link. Please ensure the link is valid and the file is public.", reply_to_message_id=original_message_id)
        except Exception:
            pass
    finally:
        if processing_msg:
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if message.voice:
        ...
        if message.document.mime_type and (message.document.mime_type.startswith('audio/') or message.document.mime_type.startswith('video/')):
            ...
            target_bot.send_message(message.chat.id, "⚠️ I can only transcribe audio or video files. Please send a valid file.", reply_to_message_id=message.message_id)
            return
    if not file_id:
        target_bot.send_message(message.chat.id, "⚠️ Unsupported file type. Send an audio file, voice message, or video.", reply_to_message_id=message.message_id)
        return
    ...
    if not user_has_stt_setting(user_id_for_settings):
        ...
        try:
            target_bot.send_message(
                message.chat.id,
                "First, please select the transcription language using the buttons below:",
...
def callback_translate_to(call):
    ...
    # Send a new "Translating" message
    translating_msg = bot.send_message(call.message.chat.id, f"Translating to **{lang_name}**...", parse_mode="Markdown")
    ...
    def do_translate():
        try:
            ...
            # Delete the "Translating" message
            try:
    ...
    bot.answer_callback_query(call.id, "Starting translation...")
...
def callback_summarize_in(call):
    ...
    # Send a new "Summarizing" message
    summarizing_msg = bot.send_message(call.message.chat.id, f"Summarizing in **{lang_name}**...", parse_mode="Markdown")
    ...
    def do_summarize():
        try:
            ...
            # Delete the "Summarizing" message
            try:
    ...
    bot.answer_callback_query(call.id, "Starting summarization...")
...
def handle_text_messages(message):
    ...
    google_drive_url = extract_google_drive_url(message.text or "")
    if google_drive_url:
        uid = str(message.from_user.id)
        if not user_has_stt_setting(uid):
            ...
            return
        threading.Thread(target=lambda: asyncio.run(process_stt_media_url(message.chat.id, uid, google_drive_url, bot, message.message_id)), daemon=True).start()
        return

    if re.search(r'https?:\/\/\S+', message.text):
        bot.send_message(message.chat.id, "❌ I only support transcription from Google Drive links. Please send a valid Google Drive link.", reply_to_message_id=message.message_id)
    else:
        bot.send_message(message.chat.id, "I don’t support text-to-Speech. If you need text-to-voice, please use this bot: https://t.me/TextToSpeechBBot", reply_to_message_id=message.message_id)


@bot.message_handler(func=lambda m: True, content_types=['sticker', 'photo', 'location', 'contact'])
def handle_unsupported_media_types(message):
    update_user_activity(message.from_user.id)
    bot.send_message(message.chat.id, "⚠️ I only transcribe audio or video. Please send an audio message, audio file, or a video.")
