...
VOICE_MAPPING = {
    ...
    # --- New Multilingual voices requested by user (synthetic ids) ---
...
def make_initial_choice_keyboard():
    ...
    markup.add(
        InlineKeyboardButton("🔥AI realistic any lang", callback_data="choice|auto"),
        InlineKeyboardButton("💗Natural single lang", callback_data="choice|by_language")
    )
    return markup
...
def make_multilingual_voices_keyboard():
    markup = InlineKeyboardMarkup(row_width=2)
    # sort multilingual voices for stable order (put common English ones first)
    def sort_key(v):
        # prefer en-US first, then en-GB, then others
    ...
    for voice in sorted(MULTILINGUAL_VOICES, key=sort_key):
        label = _multilingual_label_for_button(voice)
        markup.add(InlineKeyboardButton(label, callback_data=f"tts_voice|{voice}"))
