...
WELCOME_TEMPLATE = (
    ...
    "• Tip: For files over 20MB or if your upload is restricted, upload to Google Drive/Dropbox and send me the link.\n\n"
    "⚠️ Supported link platforms: *Facebook*, *TikTok*, *X (Twitter)* — direct links only.\n\n"
...
def help_quick_callback(call):
    help_text = (
        ...
        "Supported link platforms for large files: *Facebook*, *TikTok*, *X (Twitter)*.\n\n"
...
def help_handler(message):
    ...
    help_text = (
        ...
        "Supported link platforms: Facebook, TikTok, X. For large files, upload to Google Drive/Dropbox and send the share link.\n\n"
    ...
    try:
        ...
        if getattr(file_info, "file_size", None) and file_info.file_size > MAX_FILE_BYTES:
            target_bot.send_message(
                chat_id,
                "⚠️ The file is too large (over 20MB). Please upload it to Google Drive or Dropbox and send me a link.\n\n"
        ...
        if res.get('status') == 'completed':
            ...
            # Special guidance for Drive/Dropbox or page links masquerading as files
            lower_err = str(error_msg).lower()
            if ("file does not appear to contain audio" in lower_err) or ("transcoding failed" in lower_err) or ("text/html" in json.dumps(res).lower()):
                guidance = (
                    "❌ Transcription error: The link appears to point to a webpage or a non-audio resource.\n\n"
                    "Please make sure the file is shared publicly and send a direct file link. "
                    "For Google Drive or Dropbox links, set sharing to *Anyone with the link – Viewer* and resend the link."
                )
                target_bot.send_message(chat_id, guidance, parse_mode="Markdown", reply_to_message_id=original_message_id)
            else:
        ...
        target_bot.send_message(chat_id, "⚠️ The file is too large (over 20MB) or invalid. Please upload to Google Drive/Dropbox and send me the link.", reply_to_message_id=original_message_id)
    finally:
        if processing_msg:
            try:
...
# New: support only Facebook, TikTok, X
def is_facebook_link(text: str) -> bool:
    return bool(re.search(r"(facebook\.com/.+?/videos/|fb.watch/|m.facebook.com/.+?/videos/)", text, re.IGNORECASE))

def is_tiktok_link(text: str) -> bool:
    return bool(re.search(r"(tiktok\.com\/@.+?\/video\/|vm.tiktok.com/|tiktok.com\/t\/)", text, re.IGNORECASE))

def is_x_link(text: str) -> bool:
    return bool(re.search(r"(twitter\.com\/.+?\/status\/|x\.com\/.+?\/status\/)", text, re.IGNORECASE))
...
def process_external_audio_url(chat_id: int, user_id_for_settings: str, audio_url: str, target_bot: telebot.TeleBot, original_message_id: int):
    """
    Try letting AssemblyAI fetch the URL directly.
    If AssemblyAI can't fetch it, proxy the bytes and upload to AssemblyAI (no transcoding).
    """
    processing_msg = None
    try:
        ...
        try:
            ...
            logging.warning(f"AssemblyAI couldn't fetch external URL directly ({e}). Trying to proxy the URL bytes to AssemblyAI...")
            # Proxy the file bytes to AssemblyAI (no ffmpeg/transcoding)
            try:
                ...
                logging.error(f"Failed to fetch external URL when proxying: {e2}")
                target_bot.send_message(chat_id, "❌ Could not download the file from the provided link. Make sure the link is public and points to a direct media file.", reply_to_message_id=original_message_id)
                return
        except Exception as e:
...
def handle_text_messages(message):
    ...
    # Support only Facebook/TikTok/X links (no YouTube/Instagram)
    if is_facebook_link(text) or is_tiktok_link(text) or is_x_link(text):
        uid = str(message.from_user.id)
        if uid not in in_memory_data["stt_settings"]:
            ...
            return
        bot.send_message(message.chat.id, "🔎 Detected a supported social link (Facebook/TikTok/X). I'll attempt to fetch the media. If the post is private or requires login, upload the file to Google Drive/Dropbox and share a public link.", reply_to_message_id=message.message_id)
    ...
    # Detect Google Drive, Dropbox, direct media files
