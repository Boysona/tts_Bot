...
# Updated welcome template with cleaner UI
WELCOME_TEMPLATE = (
    "🎤 *Audio to Text Bot*\n\n"
    "Send voice/audio/video (up to 20MB) and I'll transcribe it for you!\n\n"
...
def start_handler(message):
    ...
    # Clean welcome with language buttons
    ...
    try:
        ...
        bot.send_message(message.chat.id, "Welcome! Send audio/video to get started.")
...
def open_set_output_language_callback(call):
    try:
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="🌍 *Select language:*", reply_markup=build_stt_language_keyboard(), parse_mode="Markdown")
    except Exception:
        bot.send_message(call.message.chat.id, "🌍 Select language:", reply_markup=build_stt_language_keyboard())
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data == "back_to_main")
def back_to_main_callback(call):
    uid = str(call.from_user.id)
    lang_name = next((name for name, code in STT_LANGUAGES.items() if code == get_stt_user_lang_in_memory(uid)), "English 🇬🇧")
    welcome_text = WELCOME_TEMPLATE.format(lang_name=lang_name)
    markup = InlineKeyboardMarkup()
    markup.add(
        InlineKeyboardButton("🌍 Set Language", callback_data="open_set_output_language"),
...
def help_handler(message):
    ...
    help_text = (
        "❓ *How to use:*\n\n"
        "1. Send audio/video/voice message\n"
        "2. I'll transcribe it to text\n"
        "3. Use buttons to translate/summarize\n\n"
        "📝 Tips:\n"
        "• Files must be under 20MB\n"
        "• For larger files, use Google Drive/Dropbox links\n"
        "• Set language first for best results\n\n"
        "Need help? @kookabeela"
    )
...
def send_stt_language_prompt(message):
    ...
    if message.chat.type == 'private' and str(message.from_user.id) != str(ADMIN_ID) and not check_subscription(message.from_user.id):
        send_subscription_message(message.chat.id)
        return
    bot.send_message(chat_id, "🌍 Select language:", reply_markup=build_stt_language_keyboard())
...
def on_stt_language_select(call):
    ...
    lang_name = next((name for name, code in STT_LANGUAGES.items() if code == lang_code), "Unknown")
    set_stt_user_lang_in_memory(uid, lang_code)
    bot.answer_callback_query(call.id, f"✅ {lang_name}")
    try:
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f"✅ Language set: *{lang_name}*\n\nSend audio to transcribe", parse_mode="Markdown", reply_markup=None)
    except Exception:
        bot.send_message(call.message.chat.id, f"✅ Language set: *{lang_name}*\n\nSend audio to transcribe", parse_mode="Markdown")
...
def admin_menu_callback(call):
    ...
    if call.data == "admin_total_users":
        ...
        bot.send_message(chat_id, f"📊 Users: *{total_registered}*", parse_mode="Markdown")
    elif call.data == "admin_broadcast":
        admin_state[call.from_user.id] = 'awaiting_broadcast_message'
        bot.send_message(chat_id, "✉️ Send broadcast message:")
    bot.answer_callback_query(call.id)
    ...
    try:
        ...
        if getattr(file_info, "file_size", None) and file_info.file_size > MAX_FILE_BYTES:
            target_bot.send_message(
                chat_id,
                "⚠️ File too large (>20MB). Use Google Drive/Dropbox link",
                reply_to_message_id=original_message_id
            )
            return

        # Construct a Telegram file URL
        ...
        try:
            ...
            if "text/html" in ct:
                target_bot.send_message(chat_id, "❌ Invalid file format. Try direct link", parse_mode="Markdown", reply_to_message_id=original_message_id)
                return
        except Exception:
            pass
        ...
        # Preferred: let AssemblyAI fetch the Telegram file directly
        try:
            ...
            target_bot.send_message(chat_id, "❌ Transcription failed", reply_to_message_id=original_message_id)
            return
        ...
        if res.get('status') == 'completed':
            text = res.get("text", "")
            if not text:
                target_bot.send_message(chat_id, "ℹ️ No speech detected", reply_to_message_id=original_message_id)
            elif len(text) <= 4000:
                ...
                try:
                    target_bot.send_document(chat_id, f, caption="Transcript", reply_to_message_id=original_message_id, reply_markup=markup)
                except Exception as e:
                    logging.error(f"Failed to send transcript file with buttons: {e}")
                    try:
                        target_bot.send_document(chat_id, f, caption="Transcript")
                    except Exception as ex:
                        logging.error(f"Failed fallback send: {ex}")
            increment_processing_count_in_memory(user_id_for_settings, "stt")
        else:
            error_msg = res.get("error", "Unknown error")
            target_bot.send_message(chat_id, f"❌ Error: {error_msg}", parse_mode="Markdown", reply_to_message_id=original_message_id)
            logging.error(f"AssemblyAI transcription failed: {error_msg}")
    except requests.exceptions.RequestException as e:
        logging.error(f"Network error during STT processing: {e}")
        target_bot.send_message(chat_id, "❌ Network error", reply_to_message_id=original_message_id)
    except Exception as e:
        logging.exception(f"Unhandled error during STT processing: {e}")
        target_bot.send_message(chat_id, "⚠️ File too large. Use link instead", reply_to_message_id=original_message_id)
    finally:
        if processing_msg:
            try:
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if user_id_for_settings not in in_memory_data["stt_settings"]:
        target_bot.send_message(message.chat.id, "Please set language first", reply_markup=build_stt_language_keyboard())
        return
...
def button_translate_handler(call):
    ...
    for i in range(0, len(buttons), 4):
        markup.add(*buttons[i:i+4])
    try:
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="Select target language:", reply_markup=markup)
    except Exception:
        bot.send_message(call.message.chat.id, "Select target language:", reply_markup=markup)
    bot.answer_callback_query(call.id)
...
def button_summarize_handler(call):
    ...
    for i in range(0, len(buttons), 4):
        markup.add(*buttons[i:i+4])
    try:
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="Select language:", reply_markup=markup)
    except Exception:
...
def callback_translate_to(call):
    ...
    def do_translate():
        ...
        if translated.startswith("Error:"):
            bot.send_message(call.message.chat.id, f"❌ Translation error")
            return
        if len(translated) > 4000:
            ...
            try:
                ...
                bot.send_message(call.message.chat.id, "✅ Translation complete")
        else:
...
def callback_summarize_in(call):
    ...
    def do_summarize():
        ...
        if summary.startswith("Error:"):
            bot.send_message(call.message.chat.id, f"❌ Summarization error")
            return
        if len(summary) > 4000:
            ...
            try:
                ...
                bot.send_message(call.message.chat.id, "✅ Summary complete")
        else:
...
def extract_drive_direct_link(text: str) -> str:
    ...
    m2 = re.search(r"[?&]id=([a-zA-Z00-9_-]+)", text)
    if m2:
        file_id = m2.group(1)
        return f"https://drive.google.com/uc?export=download&id={file_id}"
    return ""
...
def process_external_audio_url(chat_id: int, user_id_for_settings: str, audio_url: str, target_bot: telebot.TeleBot, original_message_id: int):
    processing_msg = None
    try:
        processing_msg = target_bot.send_message(chat_id, "🔄 Processing link...", reply_to_message_id=original_message_id)
        lang_code = get_stt_user_lang_in_memory(user_id_for_settings)
        if not lang_code:
            target_bot.send_message(chat_id, "Please set language first", reply_to_message_id=original_message_id)
            return
        ...
        try:
            ...
            target_bot.send_message(chat_id, "❌ Invalid link", reply_to_message_id=original_message_id)
            return
        ...
        logging.error(f"Network error during external STT processing: {e}")
        target_bot.send_message(chat_id, "❌ Network error", reply_to_message_id=original_message_id)
    except Exception as e:
        logging.exception(f"Unhandled error during external STT processing: {e}")
        target_bot.send_message(chat_id, "⚠️ Invalid link", reply_to_message_id=original_message_id)
    finally:
        if processing_msg:
            try:
                target_bot.delete_message(chat_id, processing_msg.message_id)
...
def handle_text_messages(message):
    ...
    if is_google_drive_link(text):
        ...
        if not direct_link:
            bot.send_message(message.chat.id, "❌ Invalid Google Drive link")
            return
        uid = str(message.from_user.id)
        if uid not in in_memory_data["stt_settings"]:
            bot.send_message(message.chat.id, "Please set language first", reply_markup=build_stt_language_keyboard())
            return
