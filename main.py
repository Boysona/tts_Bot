...
# -------------------------
# VOICE MAPPING (master)
# -------------------------
# Existing mapping kept and new multilingual voices added per your list.
VOICE_MAPPING = {
    # ... (existing mappings kept) ...
    # For brevity I include the large mapping here; this is the original mapping plus the new multilingual entries requested.
    # Core sample subset (the full original mapping from your file kept as-is above - replaced here in full in code):
    ...
    # -------------------------
    # Newly added multilingual voices (from your list)
    # Note: voice IDs follow existing naming patterns: <locale>-<Name>MultilingualNeural or ...TurboMultilingualNeural
    # -------------------------
    ...
    "en-US-BrianMultilingualNeural": "Brian - English (United States) - Multilingual",  # already existed, kept for clarity
    ...
    # French (France) - Vivienne & Remy already present as Multilingual; keep as-is
    # German (Germany) - Seraphina & Florian already present as Multilingual
    # Italian (Italy) - Giuseppe is already Multilingual
    ...
    # Thalita already present as pt-BR-ThalitaMultilingualNeural
...
# -------------------------
# TTS voices grouped by language (kept as original)
# -------------------------
TTS_VOICES_BY_LANGUAGE = {
    ...
    # ... (rest of languages kept as original) ...
...
# Get all multilingual voices (based on display string having "Multilingual")
...
def make_initial_choice_keyboard():
    # Buttons renamed per your request: Auto -> best voices, By Language -> Normal voices
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton("best voices", callback_data="choice|auto"),
...
def make_multilingual_voices_keyboard():
    """
    Multilingual voices are shown in the 'best voices' (Auto) flow.
    Buttons display as: "<Name> speaks 90+ languages"
    """
    markup = InlineKeyboardMarkup(row_width=2)
    for voice in MULTILINGUAL_VOICES:
        display_full = VOICE_MAPPING.get(voice, voice)
        # Extract short name before the first " - " if present
        short_name = display_full.split(" - ")[0]
        button_label = f"{short_name} speaks 90+ languages"
        markup.add(InlineKeyboardButton(button_label, callback_data=f"tts_voice|{voice}"))
...
def make_tts_voice_keyboard_for_language(lang_name: str):
    """
    When selecting by language (Normal voices), multilingual voices are filtered out
    so users only see the 'normal' voices per language.
    """
    markup = InlineKeyboardMarkup(row_width=2)
...
def on_initial_choice(call):
    ...
    if choice == "auto":
        # 'Auto' now labelled 'best voices' — show multilingual list with new labels
        bot.edit_message_text(
            chat_id=chat_id,
            message_id=call.message.message_id,
            text="Choose a *best voice*: 👇",
            reply_markup=make_multilingual_voices_keyboard(),
            parse_mode="Markdown"
        )
    elif choice == "by_language":
        # 'By Language' now labelled 'Normal voices'
        bot.edit_message_text(
            chat_id=chat_id,
            message_id=call.message.message_id,
...
def handle_text_for_tts_or_mode_input(message):
    ...
    update_user_activity(message.from_user.id)
    if message.chat.type == 'private' and str(message.from_user.id) != str(ADMIN_ID) and not check_subscription(message.chat.id):
        # fixed bug: call send_subscription_message (not send_subscription)
        send_subscription_message(message.chat.id)
        return
