...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        # Check file size and show Google Drive suggestion if too large
        if file_info.file_size and file_info.file_size > 20 * 1024 * 1024:
            target_bot.send_message(
                chat_id, 
                "⚠️ The file is too large. Please send one smaller than 20MB or upload to Google Drive and share the link instead.", 
                reply_to_message_id=original_message_id
            )
            return
            
        file_url = f"https://api.telegram.org/file/bot{TOKEN}/{file_info.file_path}"
        logging.info(f"Using direct file URL: {file_url}")
        
        # Use AssemblyAI's direct URL upload instead of streaming through server
        ...
        target_bot.send_message(chat_id, "❌ An error occurred during processing.", reply_to_message_id=original_message_id)
    finally:
        if processing_msg:
            try:
